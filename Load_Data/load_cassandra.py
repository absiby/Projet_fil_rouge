...
def load_meteofrance():
    ...
    session.execute('CREATE TABLE IF NOT EXISTS meteofrance_draft(numer_sta INT ,date VARCHAR,pmer VARCHAR,tend VARCHAR,cod_tend VARCHAR,dd VARCHAR,ff VARCHAR,t VARCHAR,td VARCHAR,u VARCHAR,vv VARCHAR,ww VARCHAR,w1 VARCHAR,w2 VARCHAR,n VARCHAR,nbas VARCHAR,hbas VARCHAR,cl VARCHAR,cm VARCHAR,ch VARCHAR,pres VARCHAR,niv_bar VARCHAR,geop VARCHAR,tend24 VARCHAR,tnN VARCHAR,txN VARCHAR,tminsol VARCHAR,sw VARCHAR,tw VARCHAR,raf10 VARCHAR,rafper VARCHAR,per VARCHAR,etat_sol VARCHAR,ht_neige VARCHAR,ssfrai VARCHAR,perssfrai VARCHAR,rrN VARCHAR,phenspeN VARCHAR,nnuageN VARCHAR,ctypeN VARCHAR,hnuageN VARCHAR, primary key(numer_sta, date) )')

    prepared = session.prepare("""
            INSERT INTO meteofrance_draft(numer_sta,date,pmer,tend,cod_tend,dd,ff,t,td,u,vv,ww,w1,w2,n,nbas,hbas,cl,cm,ch,pres,niv_bar,geop,tend24,tnN,txN,tminsol,sw,tw,raf10,rafper,per,etat_sol,ht_neige,ssfrai,perssfrai,rrN,phenspeN,nnuageN,ctypeN,hnuageN)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """)
    ...
    with open(lien_fichier, "r") as rows:
        ...
        for row in rows:
            if i == 1:
                ...
                session.execute(prepared, [numer_sta,date,pmer,tend,cod_tend,dd,ff,t,td,u,vv,ww,w1,w2,n,nbas,hbas,cl,cm,ch,pres,niv_bar,geop,tend24,tnN,txN,tminsol,sw,tw,raf10,rafper,per,etat_sol,ht_neige,ssfrai,perssfrai,rrN,phenspeN,nnuageN,ctypeN,hnuageN]
)
            i = 1    
